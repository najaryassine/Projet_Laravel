    ...
    <div class="col-md-6">
        <div class="form-group">
            ...
            <div class="@error('image')border border-danger rounded-3 @enderror">
                <input type="file" class="form-control-file" id="image" name="image" value="{{$project->image}}">
                @error('image')
                    <p class="text-danger text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="@error('client_id')border border-danger rounded-3 @enderror">
                <select class="form-control" id="client_id" name="client_id">
                    @foreach($users as $user)
                        @if($user->role == 1)
                            <option  value="{{$user->id}}" {{ $user->id === $project->ClientId ? 'selected' : '' }}>{{ $user->name }}</option>
                        @endif
                    @endforeach
                </select>
                @error('client_id')
...
<div class="col-md-6">
    ...
    <input class="form-control"  type="text" id="required_skills" name="required_skills" value="{{ $project->required_skills }}" >
